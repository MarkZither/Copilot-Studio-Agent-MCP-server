...
<#
.SYNOPSIS
    Build, package, and sideload the CopilotStudioAgent MCP Server VS Code extension locally.
.DESCRIPTION
...
$csproj        = Join-Path $repoRoot 'src/CopilotStudioAgent.Mcp.Server/CopilotStudioAgent.Mcp.Server.csproj'
...
if ($IsWindows) {
    $rid        = 'win-x64'
    $srcBinary  = Join-Path $publishOut 'CopilotStudioAgent.Mcp.Server.exe'
    $destBinary = Join-Path $binDir     'copilotstudioagent-mcp-server.exe'
} elseif ($IsLinux) {
    $rid        = 'linux-x64'
    $srcBinary  = Join-Path $publishOut 'CopilotStudioAgent.Mcp.Server'
    $destBinary = Join-Path $binDir     'copilotstudioagent-mcp-server-linux'
} else {
    Write-Error 'macOS is not yet supported. See https://github.com/MarkZither/copilotstudioagent-mcp-server-dotnet/issues/54'
}

Write-Host "`n=== CopilotStudioAgent MCP Server — local sideload ===" -ForegroundColor Cyan
...
try {
    ...
    $vsix = Join-Path $extensionDir 'copilotstudioagent-mcp-server.vsix'
...
Write-Host "Then set copilotstudioagent.url / copilotstudioagent.tokenId / copilotstudioagent.tokenSecret in Settings.`n"
